...
class TaskStore:
    ...
    def export_csv(self, path):
        ...
        cur.execute(
            """
            SELECT t.uuid, t.title, t.due_date, t.priority, t.status, l.name as list_name, t.project
        ...
        fieldnames = ["uuid", "title", "due_date", "priority", "status", "list_name", "project"]
    ...
    def import_csv(self, path):
        ...
        for task in rows:
            try:
                ...
                continue
            list_name = task.get("list_name") or "Imported"
            ...
            cur.execute(
                ...
                (
                    ...
                    None,
                    1 if str(task.get("priority", "")).strip().lower() in {"1", "true", "yes", "y"} else 0,
...
class FocusManager(QtWidgets.QMainWindow):
    ...
    def _project_worktree_path(self, proj):
        meta = self.project_meta.get(proj, {}) if proj else {}
        ls_path = meta.get("localsync_path") or self.localsync_paths.get(proj) if proj else None
        if meta.get("localsync") and ls_path and os.path.isdir(ls_path):
            return ls_path
        return os.path.join(PROJECT_ROOT, proj) if proj else None
    ...
    def ensure_remote_repo(self, project, dry_run=False):
        ...
        path = self._project_worktree_path(project)
        env = self.git_env()
    ...
    def update_auto_commit_toggle_state(self):
        ...
        if not proj:
            ...
            return
        path = self._project_worktree_path(proj)
        env = self.git_env()
    ...
    def update_autogit_watch(self, project, enable):
        ...
        proj_path = self._project_worktree_path(project)
        if enable:
    ...
    def _enable_auto_commit_watcher(self, project, path):
        if project in self.autogit_watchers:
            return
        if not os.path.isdir(path):
            self.log_debug("STABILITY", {"autogit_watch": "skipped", "project": project, "reason": "path-missing", "path": path})
            return
    ...
    def autogit_init(self):
        ...
        if not path:
            return
        work_path = self._project_worktree_path(proj) if proj else path
        self.run_autogit_init_for_path(work_path)
    ...
    def _auto_commit_project(self, proj, path, env=None, manual=False):
        ...
        def on_result(res):
            ...
            if msg == "clean":
                ...
                self.log_debug("VERSIONING", {"autocommit": "clean", "project": proj, "path": path, "status": stderr})
            ...
            self.log_debug("VERSIONING", {"autocommit": "success", "project": proj, "path": path})

        def on_error(err):
            if manual:
                QtWidgets.QMessageBox.critical(self, "Auto Commit", str(err))
            self.log_debug("VERSIONING", {"autocommit": "error", "project": proj, "error": str(err), "path": path})
            if manual:
    ...
    def autogit_commit(self):
        ...
        self.request_ui_mutation("autogit_commit", self._autogit_commit_safe, proj, work_path)
    ...
    def autogit_status(self):
        ...
        if not path:
            return
        path = self._project_worktree_path(proj) if proj else path
